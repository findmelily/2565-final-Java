...
import static finalhelpme.Person.*;
...
2.1) Create a constructor: Department(String title) where
    ...
    public Department(String title) {
        this.code = nextCode++;
        this.title = title != null && !title.isBlank() ? title : "Satri Si Suriyothai";
        this.staff = new Person[31];
    }
...
2.3) Create a public method: addStaff(String firstname, String lastname, int type). 
    ...
    public boolean addStaff(String firstname, String lastname, int type) {
        if (count == staff.length || !validStringCheck(firstname) || !validStringCheck(lastname)) {
            return false;
        }
